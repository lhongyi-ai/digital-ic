...
from vibfpga.board import parse_replay_image, parse_result_log, read_command, validate_profile, verify_replay_log, write_commands
...
REQUIRED_INPUTS = (
    ...
    "rtl/io/replay_source.sv", "rtl/upduino_replay.sv", "configs/upduino-replay.pcf",
...
def prepare(profile_path, bitstream_path, image_path, *, backup_path=None, backup_sha256=None, execute=False, report_path=None, constraint_path=None):
    ...
    if report.get("schema_version") != 1 or report.get("top") != "upduino_replay" or report.get("place_route_completed") is not True:
...
def run_attempt(args):
    ...
    manifest = {"schema_version": 2, "started_utc": utc_now(), "status": "preflight",
                "attempt_mode": "readback_only_recovery" if recovery else "program_and_replay",
