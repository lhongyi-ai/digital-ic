// Acoustic replay derivative; VLG1 words carry score, threshold, streak.
// Physical interface wrapper. External 12 MHz is default. INTERNAL_OSC selects
// the nominal 12 MHz HFOSC /4 for USB-only bring-up without the OSC solder jumper.
// Internal oscillator frequency has not been calibrated; cycle counts are exact,
// but time/rate conversions using 12 MHz are nominal until physically measured.
// Reset is a power-up counter; FPGA reconfiguration also restarts the design.
module upduino_acoustic #(
    parameter integer LANES=4,
    parameter integer INTERNAL_OSC=0,
    parameter integer BANDS=1,
    parameter MODEL_DIR="artifacts/model",
    parameter integer HIDDEN_SHIFT=8,
    parameter integer OUTPUT_SHIFT=7, INPUT_SHIFT=0, LOG_FEATURES=0, THRESHOLD=100, ALARM_WINDOWS=3,
   ...
   output wire status_done,status_error,status_running);
    wire system_clk;
    generate if (INTERNAL_OSC) begin : internal_clock
        SB_HFOSC #(.CLKHF_DIV("0b10")) oscillator(
            .CLKHFPU(1'b1), .CLKHFEN(1'b1), .CLKHF(system_clk));
    end else begin : external_clock
        assign system_clk=clk12;
    end endgenerate
    reg [23:0] startup=0;
    wire rst=startup<STARTUP_CYCLES;
    always @(posedge system_clk) if(rst) startup<=startup+1'b1;
    acoustic_replay_system #(.LANES(LANES),.BANDS(BANDS),.MODEL_DIR(MODEL_DIR),.HIDDEN_SHIFT(HIDDEN_SHIFT),.OUTPUT_SHIFT(OUTPUT_SHIFT),.INPUT_SHIFT(INPUT_SHIFT),.LOG_FEATURES(LOG_FEATURES),.THRESHOLD(THRESHOLD),.ALARM_WINDOWS(ALARM_WINDOWS),
        .MODEL_HASH(MODEL_HASH),.TAIL_TIMEOUT(TAIL_TIMEOUT)) system(
        .clk(system_clk),.rst(rst),.flash_miso(flash_miso),.flash_cs_n(flash_cs_n),
...
`endif
    ...
    wire [1:0] class_id;
    wire [7:0] core_error;
    acoustic_core #(.LANES(LANES),.BANDS(BANDS),.MODEL_DIR(MODEL_DIR),.HIDDEN_SHIFT(HIDDEN_SHIFT),.OUTPUT_SHIFT(OUTPUT_SHIFT),.INPUT_SHIFT(INPUT_SHIFT),.LOG_FEATURES(LOG_FEATURES),.THRESHOLD(THRESHOLD),.ALARM_WINDOWS(ALARM_WINDOWS)) core(
