...
module sensor_system #(
  ...
  output wire status_done, status_error, status_running);
    ...
    // Each raw record uses two words. Reserve the last 32 words of bank 2
    // for the latest 16 powers, instead of dedicating a fourth 16K-word RAM.
    localparam integer RAM_WORDS=3*(1<<BANK_ADDR_BITS);
    localparam integer SPECTRUM_WORD_BASE=RAM_WORDS-32;
    reg [3:0] state;
    reg [7:0] error_flags;
    ...
    // Spectrum writes have priority. A raw record's second word can wait:
    // interval_hold stays valid and no next raw record is accepted meanwhile.
    wire sample_slot=!storing && !spectrum_valid && !spectrum_storing;
    wire sample_ready=(state==RUN) && sample_slot && cal_ready && spectrum_ready;
    wire [7:0] device_id;
    ...
    static_calibration #(.OFFSET_Q8(CAL_OFFSET_Q8),.GAIN_Q20(CAL_GAIN_Q20)) calibration(
        .clk(clk),.rst(rst),.in_valid(sample_valid && state==RUN && sample_slot && spectrum_ready),
    ...
    generate if(ENABLE_SPECTRUM!=0) begin: with_spectrum
        sensor_spectrum #(.MODEL_DIR(SPECTRUM_MODEL_DIR)) spectrum(
            .clk(clk),.rst(rst),.in_valid(sample_valid && state==RUN && sample_slot && cal_ready),
        ...
        assign spectrum_index=0;assign spectrum_power=0;assign spectrum_clips=0;
    end endgenerate
    ...
    wire spectrum_write=spectrum_valid || spectrum_storing;
    wire raw_write=!spectrum_write && (storing || (accept_sample && stored_count<16'(STORE_SAMPLES)));
    wire ram_write=spectrum_write || raw_write;
    wire [15:0] store_address={stored_count[14:0],1'b0}+{15'd0,storing};
    wire [4:0] spectrum_word=spectrum_valid ? {spectrum_index,1'b0} : {spectrum_saved_index,1'b1};
    // Header bytes 144..207 retain the existing SEN1 wire layout. The power
    // cache's physical address is private; only its 5-bit offset changes here.
    wire [4:0] spectrum_read_word=write_position[5:1]-5'd8;
    wire reading_spectrum=ENABLE_SPECTRUM!=0 && write_kind==1 && write_position>=144;
    wire [15:0] ram_address=spectrum_write ? (16'(SPECTRUM_WORD_BASE) | {11'd0,spectrum_word}) :
        raw_write ? store_address : reading_spectrum ?
        (16'(SPECTRUM_WORD_BASE) | {11'd0,spectrum_read_word}) : write_position[16:1];
    wire [1:0] ram_bank=2'(ram_address >> BANK_ADDR_BITS);
    wire [13:0] local_address=14'(ram_address & ((1<<BANK_ADDR_BITS)-1));
    wire [15:0] ram_data=spectrum_write ? (spectrum_valid ? spectrum_power[15:0] : spectrum_high) :
        storing ? interval_hold : selected_axis;
    wire [15:0] q0,q1,q2;
            ...
            WRITE_DATA: begin
                ...
                if(write_kind==0) wr_data=write_position[0] ? ram_q[15:8] : ram_q[7:0];
                    ...
                    if(ENABLE_SPECTRUM!=0 && write_position>=144)
                        wr_data=spectrum_windows==0 ? 8'd0 : write_position[0] ? ram_q[15:8] : ram_q[7:0];
    ...
    always @(posedge clk) begin
        if(rst) begin
            ...
            if(storing && !spectrum_write) begin storing<=0; stored_count<=stored_count+1'b1; end
            ...
            case(state)
                SCAN_CMD: if(TOTAL_SAMPLES<1 || STORE_SAMPLES<1 || STORE_SAMPLES>TOTAL_SAMPLES ||
                    BANK_ADDR_BITS<5 || BANK_ADDR_BITS>14 ||
                    STORE_SAMPLES*2>RAM_WORDS-(ENABLE_SPECTRUM!=0 ? 32 : 0) || STORE_SAMPLES>24000 ||
...
`ifdef VIB_ASSERT
    always @(posedge clk) if(!rst) begin
        // The current DSP emits powers several cycles apart; each power owns
        // two consecutive RAM write slots. Detect a future producer change.
        assert(!(spectrum_valid && spectrum_storing));
        if(spectrum_write) begin
            assert(!accept_sample);
            assert(ram_bank==2 && ram_address>=16'(SPECTRUM_WORD_BASE));
        end
        if(raw_write && ENABLE_SPECTRUM!=0) assert(ram_address<16'(SPECTRUM_WORD_BASE));
    end
`endif
endmodule
