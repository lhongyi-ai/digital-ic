// Acoustic derivative: normal-only reconstruction, squared-error score, and temporal alarm.
...
module acoustic_core #(
    ...
    parameter integer OUTPUT_SHIFT = 7,
    parameter integer INPUT_SHIFT = 0,
    parameter integer LOG_FEATURES = 0,
    parameter integer THRESHOLD = 100,
    parameter integer ALARM_WINDOWS = 3
) (
    input wire clk,
    input wire rst,
    input wire in_valid,
    ...
    localparam integer W2_GROUPS = (16+LANES-1)/LANES;
    ...
    localparam integer NN_MAX = (16/LANES+W2_GROUPS)*(49+LANES)+32;
    ...
    localparam [4:0] IDLE=0, PRE_MEAN=1, PRE_READ=2, PRE_MUL=3,
        ...
        NN_MUL=19, NN_ACC=20, NN_STORE=21, FINISH=22, HOLD=23, ERR_MUL=24, ERR_ADD=25;
    reg [4:0] state;
    ...
    reg [7:0] reconstruction [0:15];
    reg [3:0] error_index;
    reg [17:0] score;
    localparam integer STREAK_W=$clog2(ALARM_WINDOWS+1);
    reg [STREAK_W-1:0] streak;
    reg previous_valid;
    reg [31:0] previous_id;
    wire consecutive=previous_valid && active_id==previous_id+1;
    wire [STREAK_W-1:0] next_streak=(arithmetic_errors!=0 || score<=THRESHOLD) ? 0 :
        !consecutive ? STREAK_W'(1) : streak<ALARM_WINDOWS ? streak+1'b1 : streak;
    wire [31:0] log_code = quant_work>=4 ? ((32'(quant_remaining)+2)*4+32'(quant_work)-4) :
        quant_work>=2 ? 4+(32'(quant_work)-2)*2 : 0;
    wire [7:0] acoustic_feature = LOG_FEATURES ? (log_code>127 ? 8'd127 : 8'(log_code)) : quant_result;
    ...
    function automatic [7:0] output_quant;
        input signed [39:0] value;
        reg signed [39:0] rounded;
        begin
            rounded=rne(value,OUTPUT_SHIFT);
            if(rounded<0) output_quant=0;
            else if(rounded>127) output_quant=127;
            else output_quant=rounded[7:0];
        end
    endfunction
    ...
    always @* begin
        ...
        case (state)
            PRE_MUL: begin
                multiply_enable=1;
                operand_a[0]=sat12(rne($signed({{24{raw_q[15]}},raw_q})-$signed({{24{mean[15]}},mean}),INPUT_SHIFT));
                operand_b[0]=hann_q;
            end
            DFT_MUL: begin
                multiply_enable=1;
            ...
            ERR_MUL: begin
                multiply_enable=1;
                operand_a[0]=$signed({8'd0,features[error_index]})-$signed({8'd0,reconstruction[error_index]});
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            error_index<=0; score<=0; streak<=0; previous_valid<=0; previous_id<=0;
        end else begin
            dbg_valid<=0;
            ...
            if ((state >= NN_INIT && state <= NN_STORE) || state==ERR_MUL || state==ERR_ADD) count_nn<=count_nn+1;
            ...
            case (state)
                IDLE: if (full[read_bank]) begin
                    ...
                    count_pre<=0; count_dft<=0; count_power<=0; count_nn<=0; count_total<=0; score<=0; error_index<=0;
                    state<=PRE_MEAN;
                end
                PRE_MEAN: begin
                ...
                POWER_Q_INIT: begin
                    ...
                    if (LOG_FEATURES) begin
                        quant_remaining<=0;
                        if (power_sum<8) state<=POWER_QUANT; else state<=POWER_Q_SHIFT;
                    end else if (feature_shifts[feature_index] == 0) state<=POWER_QUANT;
                    else if (feature_shifts[feature_index] > 33) begin quant_work<=0; state<=POWER_QUANT; end
                    else state<=POWER_Q_SHIFT;
                end
                POWER_Q_SHIFT: if (LOG_FEATURES) begin
                    quant_work<=quant_work>>1; quant_remaining<=quant_remaining+1;
                    if (quant_work<16) state<=POWER_QUANT;
                end else begin
                ...
                POWER_QUANT: begin
                    features[feature_index]<=acoustic_feature;
                    dbg_valid<=1; dbg_kind<=5; dbg_index<={12'd0,feature_index}; dbg_value<=$signed({32'd0,acoustic_feature});
                ...
                NN_INIT: begin
                    ...
                    for (k=0; k<LANES; k=k+1) begin
                        ...
                        else if (int'(nn_group)+k < 16) accumulator[k]<=40'(bias2[int'(nn_group)+k]);
                ...
                NN_ACC: begin
                    ...
                    if (nn_input == (nn_layer ? 7 : 15)) begin store_lane<=0; state<=NN_STORE; end
                ...
                NN_STORE: begin
                    ...
                    if (!nn_layer) begin
                        ...
                        reconstruction[nn_output_index]<=output_quant(selected_accumulator);
                        dbg_valid<=1; dbg_kind<=7; dbg_index<={12'd0,nn_output_index}; dbg_value<=selected_accumulator;
                    end
                    if (store_lane == LB'(LANES-1)) begin
                        if (!nn_layer && int'(nn_group)+LANES == 8) begin nn_layer<=1; nn_group<=0; state<=NN_INIT; end
                        else if (nn_layer && int'(nn_group)+LANES >= 16) state<=ERR_MUL;
                        else begin nn_group<=nn_group+4'(LANES); state<=NN_INIT; end
                    end else store_lane<=store_lane+1;
                end
                ERR_MUL: begin
                    dbg_valid<=1; dbg_kind<=8; dbg_index<={12'd0,error_index}; dbg_value<=40'(reconstruction[error_index]);
                    state<=ERR_ADD;
                end
                ERR_ADD: begin
                    score<=score+18'(product[0]);
                    dbg_valid<=1;dbg_kind<=9;dbg_index<={12'd0,error_index};dbg_value<=40'(product[0]);
                    if(error_index==15) state<=FINISH;
                    else begin error_index<=error_index+1;state<=ERR_MUL;end
                end
                FINISH: begin
                    out_valid<=1; out_frame_id<=active_id;
                    // Existing fixed-size log transports score, threshold, and streak.
                    out_logits<={32'(next_streak),32'(THRESHOLD),32'(score)};
                    out_class<=next_streak>=ALARM_WINDOWS ? 1 : 0;out_error<=arithmetic_errors;
                    streak<=next_streak;previous_id<=active_id;previous_valid<=1;
...
`ifdef VIB_ASSERT
    initial begin
        assert (ALARM_WINDOWS>=1 && OUTPUT_SHIFT>=0 && OUTPUT_SHIFT<40);
        assert (N >= 8 && (N & (N-1)) == 0);
