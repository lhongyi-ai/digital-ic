...
class NorPins(SpiPeripheral):
    def __init__(self,dut):
        ...
        self.nor=SpiNor(dut,cycle=lambda:self.tick_count)

    def start(self):
        return self.nor.begin()
...
class SystemHarness:
    def __init__(self,dut):
        ...
        self.storage_stress=os.environ["SENSOR_CASE"]=="spectrum_storage"
        self.raw_deferred=0;self.sample_deferred=0
    ...
    async def tick(self,reset=False):
        ...
        if self.storage_stress and not reset:
            writing=int(self.dut.spectrum_write.value)
            self.raw_deferred+=int(writing and int(self.dut.storing.value))
            self.sample_deferred+=int(writing and int(self.dut.sample_valid.value))
    ...
    async def sample_run(self,total,*,overflow=False):
        ...
        for index in range(total):
            if self.storage_stress and index in (256,512):
                # Align an actual pin-level SPI read with the first spectral
                # result: raw pair completion first, pending sample next time.
                # N256/L1: PRE=769, DFT=24640; include IDLE and power stages.
                target=self.cycles+25414+(index==512)
                next_due=target-service_latency
...
async def capture_to_committed_flash(dut):
    ...
    spectrum=scenario.startswith("spectrum")
    header_bytes=208 if spectrum else 128
    h=SystemHarness(dut)
    protected=hashlib.sha256(h.flash.nor.memory[:BASE]).hexdigest()
    await h.tick(reset=True)
    if scenario=="spectrum_overlap":
        await h.wait(lambda:int(dut.state.value)==9)
        assert int(dut.error_flags.value)==128
        assert int(dut.observed_count.value)==0 and not int(dut.status_done.value)
        assert not h.flash.nor.operations and not h.sensor.transactions
        Path(os.environ["SENSOR_REPORT"]).write_text(json.dumps({
            "passed":True,"case":scenario,"overlapping_allocation_rejected":True,
            "total_samples":total,"stored_samples":retained,"hardware_measured":False},indent=2)+"\n")
        return
    ...
    if not overflow and total>=80:assert wraps==1, "The long runs must cross the 32-bit cycle rollover"
    ...
    if spectrum:
        ...
        windows=total//256
        assert metadata==(windows,256,1,0)
        ...
        if windows:
            expected=frontend(np.clip(np.array(raw[(windows-1)*256:windows*256],dtype=np.int64),-1024,1023)*32,model)
            assert spectral_powers==[int(value) for value in expected["powers"]]
        else:assert spectral_powers==[0]*16
        if scenario=="spectrum_storage":
            assert h.raw_deferred>0, "Must exercise a raw timestamp deferred by spectrum writes"
            assert h.sample_deferred>0, "Must exercise sample backpressure during spectrum writes"
            assert retained*2==3*256-32, "Raw allocation must end exactly before the spectrum cache"
    if scenario=="nominal":
    ...
    report={"passed":True,"case":scenario,
        ...
        "raw_timestamp_deferred_cycles":h.raw_deferred,"sample_deferred_cycles":h.sample_deferred,
        "spectrum_allocation_boundary":scenario=="spectrum_storage",
