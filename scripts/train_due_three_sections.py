#!/usr/bin/env python3
"""Minimal supervised DUE 00/01/02 whole-section development experiment."""
...
def main():
    out = ROOT / "artifacts/acoustic-due-three-sections-v1"
    out.mkdir(exist_ok=False)
    plan_path = BASE / "expansion-plan.json"
    plan = json.loads(plan_path.read_text())
    for receipt_name in ("development-download.json", "section02-development-download.json"):
        receipt = json.loads((BASE / receipt_name).read_text())
        assert receipt["passed"] and receipt["plan_sha256"] == sha(plan_path)
    rows = [dict(row) for row in plan["records"] if row["role"] in ("fit", "calibration") or
        (row["section"] == "02" and "_test_" in row["name"])]
    for domain in ("source", "target"):
        for label in (0, 1):
            group = sorted((row for row in rows if row["section"] == "02" and
                row["domain"] == domain and row["label"] == label),
                key=lambda row: hashlib.sha256(("due-v2:" + row["name"]).encode()).hexdigest())
            assert len(group) == 100
            for index, row in enumerate(group):
                row["role"] = "fit" if index < 80 else "calibration"
    assert len(rows) == 1200 and {row["section"] for row in rows} == {"00", "01", "02"}
    ...
    dump(out / "protocol.json", {"plan_sha256": sha(plan_path),
        ...
        "models": ["rbf", "trees"], "fold": "hold out section 00, 01, or 02 in full",
        "threshold": "maximum source-section normal calibration score",
        "gate": "recall > .90, fpr < .05, auc >= .90 on all three sections",
        "test": "DCASE 2021 evaluation sections 03/04/05 unopened", "source_sha256": sha(Path(__file__))})
