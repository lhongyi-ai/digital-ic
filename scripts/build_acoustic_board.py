...
def main():
    ...
    p.add_argument("--model", type=Path, default=ROOT / "artifacts/acoustic/model/model.json")
    p.add_argument("--seed", type=int, default=1)
    p.add_argument("--clock-source", choices=("external12", "hfosc12"), default="hfosc12")
    add_run_arguments(p)
    a = p.parse_args()
    ...
    out = run_path(a, f"acoustic_board_{'neighbor_' if bands==3 else ''}l{a.lanes}{'_hfosc12' if a.clock_source == 'hfosc12' else ''}")
    out.mkdir(parents=True, exist_ok=True)
    sources = ["rtl/core/vib_coeff_rom.sv", "rtl/core/acoustic_core.sv", "rtl/core/sync_fifo.sv",
               "rtl/platform/spram16k.sv", "rtl/io/spi_master.sv", "rtl/io/flash_stream.sv",
               "rtl/io/replay_source.sv", "rtl/upduino_acoustic.sv"]
    ...
    inputs={(str(p.relative_to(ROOT)) if p.is_relative_to(ROOT) else str(p)): hashlib.sha256(p.read_bytes()).hexdigest()
            for p in [*(ROOT / s for s in sources),ROOT / "configs/upduino-replay.pcf",pcf,Path(__file__).resolve(),ROOT/"src/vibfpga/acoustic.py",ROOT/"src/vibfpga/acoustic_board.py",ROOT/"src/vibfpga/fixed.py",*([clock_script] if a.clock_source == "hfosc12" else []),*model_path.parent.glob("*.hex"),model_path]}
    ...
    script = ("read_verilog -defer -sv -DICE40 " + " ".join(sources) + ";\n"
              ...
              f'-set OUTPUT_SHIFT {model["output_shift"]} -set INPUT_SHIFT {model["input_shift"]} -set LOG_FEATURES {int(model["acoustic_encoding"]=="log4")} -set THRESHOLD {model["threshold"]} -set ALARM_WINDOWS {model["alarm_windows"]} -set HIDDEN_SHIFT {model["hidden_shift"]} -set INTERNAL_OSC {int(a.clock_source == "hfosc12")} '
              f"-set MODEL_HASH 256'h{digest[::-1].hex()} upduino_acoustic;\n"
              f"synth_ice40 -dsp -top upduino_acoustic -json {yosys_quote(out / 'netlist.json')};\nstat\n")
    ...
    cells = design["modules"]["upduino_acoustic"]["cells"]
    counts = {}
    ...
    report = {"schema_version": 1, "top": "upduino_acoustic", "lanes": a.lanes,"bands":bands,
