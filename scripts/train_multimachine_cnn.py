...
class Net(nn.Module):
    def __init__(self):
        super().__init__();self.net=nn.Sequential(nn.Conv2d(1,8,3,padding=1),nn.ReLU(),nn.AvgPool2d(2),nn.Conv2d(8,16,3,padding=1),nn.ReLU(),nn.AvgPool2d(2),nn.Conv2d(16,16,3,padding=1),nn.ReLU(),nn.AdaptiveAvgPool2d(1),nn.Flatten(),nn.Linear(16,1))
...
def patches(pcm):
    ...
    # Four hops per feature column: 64 ms. Four disjoint 2.048 s patches.
    energy=energy[:512].reshape(128,4,32).mean(1)
...
def predict(model,x):
    ...
    with torch.no_grad():
        for start in range(0,len(x),32):
            a=x[start:start+32];v=model(torch.from_numpy(a.reshape(-1,1,32,32))).reshape(-1,4).mean(1);values.extend(v.numpy().tolist())
    return np.array(values)

def main():
    torch.set_num_threads(2);torch.use_deterministic_algorithms(True)
    out=ROOT/'artifacts/acoustic-cnn-v1';out.mkdir(exist_ok=False)
    ...
    protocol={'source_sha256':sha(Path(__file__)),'plan_sha256':sha(pp),'torch':torch.__version__,'seed':43,'epochs':30,'batch':64,'learning_rate':.002,
      'representations':['relative','timecenter'],'selection':'last epoch fixed; held machine never used for stopping','score':'mean four patch logits per recording','input':'four disjoint 2.048 s spectrogram patches from first 8.192 s; 32 uniform bands','threshold':'max source-machine normal calibration threshold','test_opened':False}
    dump(out/'protocol.json',protocol);xx=[]
    for i,r in enumerate(rows):
        p=ROOT/r['local'];assert sha(p)==f['pcm_sha256'][r['local']];xx.append(patches(np.load(p)))
        if (i+1)%240==0:print('features',i+1,flush=True)
    raw=np.stack(xx);np.save(out/'patches.npy',raw);results=[]
    for representation in protocol['representations']:
        # Each record/patch normalizes itself, with no population or target statistics.
        x=raw-raw.mean(axis=(2,3),keepdims=True)
        if representation=='timecenter':x=x-x.mean(axis=3,keepdims=True)
        x=np.ascontiguousarray(x/4,dtype=np.float32);rr=[]
        for held,fit,cal,val in folds(rows):
            ...
            for epoch in range(30):
                ...
                for start in range(0,len(order),64):
                    idx=order[start:start+64];patch=rng.integers(0,4,len(idx));a=x[idx,patch,None,:,:];target=torch.from_numpy(y[idx].astype(np.float32));opt.zero_grad();loss=lossfn(model(torch.from_numpy(a)),target);loss.backward();opt.step();losses.append(float(loss.detach()))
            ...
            r.update(machine=held,fit_indices=fit.tolist(),calibration_indices=cal.tolist(),validation_indices=val.tolist(),validation_scores=vs.tolist(),calibration_scores=cs.tolist(),training_loss=history,gate_passed=r['recall']>.9 and r['fpr']<.05 and r['auc']>=.9);rr.append(r)
