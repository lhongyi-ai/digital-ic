#!/usr/bin/env python3
"""Fixed train/validation-only feature study. No test evaluation entry point."""
import argparse
import copy
import hashlib
import json
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--output',default='artifacts/acoustic-v2-study');a=p.parse_args()
    ...
    protocol={'plan_sha256':ph,'splits':['train','validation'],'test_opened':False,'seed':7,
      ...
      'normalization':'log1p band energy; means/std train normal only; centroid unstandardized, diagonal standardized, AE standardized',
    ...
    for kind,b in [('sparse',16),('uniform',16),('uniform',32),('mel',16),('mel',32)]:
        bank=filterbank(b,kind);tf=np.log1p(pt@bank.T).astype(np.float32);vf=np.log1p(pv@bank.T).astype(np.float32)
        for k in [1,4,8]:
